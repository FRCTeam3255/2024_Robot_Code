...
public class AddVisionMeasurement extends Command {
  ...
  public void execute() {
    ...
    // arRotRelativeToBot = Math.min(Math.abs(drivetrainRotation -
    // arRotation.getZ()),
    // 360 - Math.abs(drivetrainRotation - arRotation.getZ()));

    // // Calculate angular distance considering wrapping around 360 degrees
    // ovRotRelativeToBot = Math.min(Math.abs(drivetrainRotation -
    // ovRotation.getZ()),
  ...
  // if (estimatedPose.getX() < 5.847097873687744 || estimatedPose.getX() >
  // 10.699200630187988) {
  // return true;
  // }
  // }
