...
public class ZeroIntake extends Command {
  ...
  public void initialize() {
    subIntake.setIntakeSoftwareLimits(true, false);

    subIntake.setPivotMotorVoltage(0);
    zeroingTimestamp = 0;
  }
  ...
  @Override
  public void execute() {
    subIntake.setPivotMotorVoltage(prefIntake.intakeZeroingVoltage.getValue());
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {
    subIntake.setIntakeSoftwareLimits(true, true);
    ...
    if (!interrupted) {
      subIntake.setIntakeSensorAngle(prefIntake.intakeSensorZeroedAngle.getValue());
    }
  }
  ...
  public boolean isFinished() {
    ...
    if (Math.abs(subIntake.getIntakeVelocity()) <= Math.abs(prefIntake.intakeZeroedVelocity.getValue())) {
      ...
      // long enough
      return (Timer.getFPGATimestamp() - zeroingTimestamp) >= prefIntake.intakeZeroedTime.getValue();
    }
