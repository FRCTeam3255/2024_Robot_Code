...
public class UpperCenterline extends SequentialCommandGroup implements AutoInterface {
  ...
  public UpperCenterline(Drivetrain subDrivetrain, Intake subIntake, LEDs subLEDs, Pitch subPitch, Shooter subShooter,
    ...
    addCommands(
        ...
        AutoBuilder.followPath(PathPlannerPath.fromChoreoTrajectory("D C1UntilC5.1"))
            .unless(() -> lastGamePiece == 4),
        ...
        Commands.waitSeconds(prefIntake.intakeGamePieceGetTime.getValue()),
        new SequentialCommandGroup(
            Commands.runOnce(() -> lastGamePiece = 5),
            AutoBuilder.followPath(PathPlannerPath.fromChoreoTrajectory("D C1sUntilC5s.3")),
            new Shoot(subShooter, subLEDs).until(() -> !subTransfer.calcGamePieceCollected()),
            AutoBuilder.followPath(PathPlannerPath.fromChoreoTrajectory("D C1sUntilC5s.4")))
            .unless(() -> !subTransfer.calcGamePieceCollected()),

        // If no, drive to collect C3 from C4
        AutoBuilder.followPath(PathPlannerPath.fromChoreoTrajectory("D C1UntilC5.2"))
            .unless(() -> lastGamePiece == 5),
        ...
        new SequentialCommandGroup(
            ...
            AutoBuilder.followPath(PathPlannerPath.fromChoreoTrajectory("D C1sUntilC5s.5")),
            new Shoot(subShooter, subLEDs).until(() -> !subTransfer.calcGamePieceCollected())));
  }
