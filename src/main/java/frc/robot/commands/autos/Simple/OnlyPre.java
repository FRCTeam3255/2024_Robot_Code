...
public class OnlyPre extends SequentialCommandGroup implements AutoInterface {
  ...
  public OnlyPre(Drivetrain subDrivetrain, Intake subIntake, LEDs subLEDs, Pitch subPitch, Shooter subShooter,
    ...
    addCommands(
        Commands.runOnce(
            () -> subDrivetrain.resetPoseToPose(PathPlannerAuto.getStaringPoseFromAutoFile("OnlyW2"))),
        Commands.runOnce(() -> subDrivetrain.resetYaw(
            PathPlannerAuto.getStaringPoseFromAutoFile("OnlyW2").getRotation().getDegrees())),
  ...
  public Supplier<Pose2d> getInitialPose() {
    return () -> (!FieldConstants.isRedAlliance())
        ? PathPlannerAuto.getStaringPoseFromAutoFile("OnlyW2")
        : PathPlannerPath.fromPathFile("OnlyW2").flipPath().getPreviewStartingHolonomicPose();
  }
