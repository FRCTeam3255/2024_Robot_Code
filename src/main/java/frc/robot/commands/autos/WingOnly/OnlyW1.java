...
public class OnlyW1 extends SequentialCommandGroup implements AutoInterface {
  ...
  public OnlyW1(Drivetrain subDrivetrain, Intake subIntake, LEDs subLEDs, Pitch subPitch, Shooter subShooter,
    ...
    addCommands(
        Commands.runOnce(
            () -> subDrivetrain.resetPoseToPose(PathPlannerAuto.getStaringPoseFromAutoFile("OnlyW1"))),
        Commands.runOnce(() -> subDrivetrain.resetYaw(
            PathPlannerAuto.getStaringPoseFromAutoFile("OnlyW1").getRotation().getDegrees())),
  ...
  public Supplier<Pose2d> getInitialPose() {
    return () -> (!FieldConstants.isRedAlliance())
        ? PathPlannerAuto.getStaringPoseFromAutoFile("OnlyW1")
        : PathPlannerPath.fromPathFile("OnlyW1").flipPath().getPreviewStartingHolonomicPose();
  }
